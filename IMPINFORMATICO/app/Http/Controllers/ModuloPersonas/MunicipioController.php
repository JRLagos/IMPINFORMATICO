...
class MunicipioController extends Controller {
    ...
    */
    ...
    public function update(Request $request)
    {
        ...
        $existingMunicipio = Http::get('http://localhost:3000/SHOW_MUNICIPIO/GETALL_MUNICIPIO/0', [
            "COD_DEPARTAMENTO" => $request->input("COD_DEPARTAMENTO"),
            "NOM_MUNICIPIO" => $request->input("NOM_MUNICIPIO"),
        ]);
    
        if ($existingMunicipio->getStatusCode() !== 200) {
            // Hubo un error en la solicitud a la API
            return redirect()->back()->with('error', 'Error al verificar la existencia del municipio.');
        }
    
        $existingMunicipioData = $existingMunicipio->json();
    
        if (!empty($existingMunicipioData) && count($existingMunicipioData) > 0) {
            // Ya existe un municipio con el mismo nombre en el mismo departamento
            return redirect()->back()->with('error', 'Ya existe un municipio con el mismo nombre en este departamento.');
        }
    
        // Si no existe, procede con la actualización
